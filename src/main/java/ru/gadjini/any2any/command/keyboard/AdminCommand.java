...
public class AdminCommand implements KeyboardBotCommand, NavigableBotCommand {
    ...
    public boolean processMessage(Message message, String text) {
        ...
        messageService.sendMessage(
                SendMessage.builder().chatId(String.valueOf(message.getChatId()))
                        .text(localisationService.getMessage(MessagesProperties.ADMIN_COMMAND_NAME, locale))
                        .replyMarkup(replyKeyboardService.getAdminKeyboard(message.getChatId(), locale)).build()
        );
    ...
    public void processNonCommandUpdate(Message message, String text) {
        Locale locale = userService.getLocaleOrDefault(message.getFrom().getId());
        if (text.equals(localisationService.getMessage(MessagesProperties.REMOVE_GARBAGE_FILES_COMMAND_NAME, locale))) {
            int deleted = garbageFileCollector.clean();
            messageService.sendMessage(new SendMessage(String.valueOf(message.getChatId()),
                    localisationService.getMessage(MessagesProperties.MESSAGE_COLLECTED_GARBAGE, new Object[]{deleted}, locale)));
        } else {
